...
function NavLink({ label, active, onClick }) {
    ...
    return (
        <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            style={{ fontFamily: "'Syne',sans-serif", fontSize: "0.83rem", fontWeight: 600, border: "none", cursor: "none", color: active ? "#fff" : hov ? C.coral : C.inkSoft, background: active ? C.coral : hov ? "rgba(255,60,40,0.07)" : "transparent", padding: "0.4rem 1rem", borderRadius: 50, boxShadow: active ? "0 3px 12px rgba(255,60,40,0.28)" : "none", transition: "all 0.2s" }}>
            {label}
        </button>
    );
}
