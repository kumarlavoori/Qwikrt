...
function BtnCoral({ children, onClick, style = {} }) {
    ...
    return (
        <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "clamp(0.8rem,1.5vw,0.9rem)", background: hov ? C.coralSoft : C.coral, color: "#fff", border: "none", cursor: "none", padding: "0.88rem 1.9rem", borderRadius: 50, boxShadow: hov ? "0 10px 28px rgba(255,60,40,0.38)" : "0 4px 18px rgba(255,60,40,0.28)", display: "inline-flex", alignItems: "center", gap: "0.5rem", transform: hov ? "translateY(-2px)" : "translateY(0)", transition: "all 0.22s", letterSpacing: "0.02em", whiteSpace: "nowrap", ...style }}>
            {children}
        </button>
    );
}
...
function ViewAllBtn({ children, onClick }) {
    ...
    return (
        <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            style={{ background: hov ? C.purple : "none", border: `1.5px solid ${hov ? C.purple : "rgba(61,26,120,0.18)"}`, color: hov ? "#fff" : C.purple, fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "0.85rem", padding: "0.6rem 1.5rem", borderRadius: 50, cursor: "none", transition: "all 0.2s" }}>
            {children}
        </button>
    );
}
...
/* ════════════ MARQUEE ════════════ */
function Marquee({ items, coral = false, dark = false, reverse = false }) {
    const tripled = [...items, ...items, ...items];
    const bg = coral ? C.coral : dark ? C.purpleDark : C.creamDark;
    const itemColor = coral ? "rgba(255,255,255,0.85)" : dark ? "rgba(247,243,236,0.38)" : C.inkSoft;
    const sepColor = coral ? "rgba(255,255,255,0.5)" : C.coral;
    return (
...
const CAT_COLORS = {
    Frontend: { bg: "rgba(61,26,120,0.08)", border: "rgba(61,26,120,0.2)", text: "#3D1A78", dot: "#3D1A78" },
    Backend: { bg: "rgba(255,60,40,0.07)", border: "rgba(255,60,40,0.22)", text: "#C03000", dot: "#FF3C28" },
    Design: { bg: "rgba(255,186,8,0.1)", border: "rgba(255,186,8,0.3)", text: "#9A6E00", dot: "#FFBA08" },
    Mobile: { bg: "rgba(0,160,100,0.08)", border: "rgba(0,160,100,0.22)", text: "#006640", dot: "#00A064" },
    CMS: { bg: "rgba(107,63,168,0.1)", border: "rgba(107,63,168,0.28)", text: "#6B3FA8", dot: "#6B3FA8" },
...
function WhyCard({ Ico, title, desc }) {
    ...
    return (
        <div data-hover onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            style={{ display: "flex", gap: "1.2rem", padding: "1.5rem", borderRadius: 16, background: "#fff", border: `1px solid ${hov ? C.purpleSoft : C.creamDark}`, transform: hov ? "translateX(8px)" : "translateX(0)", boxShadow: hov ? "0 8px 30px rgba(61,26,120,0.09)" : "none", transition: "all 0.3s" }}>
            <div style={{ width: 48, height: 48, borderRadius: 12, flexShrink: 0, background: "rgba(61,26,120,0.07)", display: "flex", alignItems: "center", justifyContent: "center", color: C.purple }}><Ico /></div>
            <div>
        ...
        <div data-hover className="svc-card-white" onMouseEnter={onEnter} onMouseLeave={onLeave} onClick={onClick}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", marginBottom: "1.1rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: active ? "rgba(61,26,120,0.1)" : "rgba(61,26,120,0.06)", display: "flex", alignItems: "center", justifyContent: "center", color: active ? C.purple : C.inkSoft, transition: "all 0.2s" }}>
                        <s.Ico />
                    </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.38rem" }}>
                {s.tags.map((t) => (
                    <span key={t} style={{ fontFamily: "'Syne',sans-serif", fontSize: "0.63rem", fontWeight: 700, color: active ? C.purple : C.inkSoft, background: active ? "rgba(61,26,120,0.08)" : C.creamDark, padding: "0.2rem 0.6rem", borderRadius: 50, transition: "all 0.2s" }}>{t}</span>
                ))}
            </div>
    ...
    return (
        <div
            ...
            style={{
                ...
                border: `1.5px solid ${hov ? "rgba(255,60,40,0.55)" : "rgba(255,60,40,0.2)"}`,
                ...
                boxShadow: hov
                    ? "0 20px 48px rgba(255,60,40,0.18), 0 4px 16px rgba(0,0,0,0.2)"
        ...
        >
            ...
            <div style={{
                ...
                background: hov ? "rgba(255,60,40,0.2)" : "rgba(255,60,40,0.12)",
                border: `1.5px solid rgba(255,60,40,${hov ? "0.45" : "0.25"})`,
                display: "flex", alignItems: "center", justifyContent: "center",
                color: C.coral,
                marginBottom: "1.4rem",
                boxShadow: hov ? "0 0 18px rgba(255,60,40,0.3)" : "none",
            ...
            <div style={{
                ...
                background: `linear-gradient(90deg, ${C.coral}, rgba(255,60,40,0))`,
                width: hov ? "100%" : "36%",
...
function ProcCard({ num, title, desc }) {
    ...
    return (
        <div data-hover onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            style={{ background: "#fff", borderRadius: 20, padding: "2rem", border: `1px solid ${C.creamDark}`, position: "relative", overflow: "hidden", transform: hov ? "translateY(-4px)" : "none", boxShadow: hov ? "0 16px 40px rgba(61,26,120,0.1)" : "none", transition: "all 0.3s", height: "100%", boxSizing: "border-box" }}>
            ...
            <div style={{ width: 40, height: 40, borderRadius: 12, background: "rgba(61,26,120,0.07)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "0.85rem", color: C.purple, marginBottom: "1.2rem" }}>{num}</div>
...
function IndCard({ icon, name, desc, tag }) {
    const [hov, setHov] = useState(false);
    return (
        <div data-hover onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            style={{
                background: hov ? C.coral : "#fff",
                borderRadius: 16, padding: "1.6rem",
                border: `1px solid ${hov ? C.coral : C.creamDark}`,
                transform: hov ? "translateY(-5px)" : "none",
                boxShadow: hov ? "0 16px 40px rgba(255,60,40,0.22)" : "none",
            ...
            <div style={{
                ...
                color: hov ? "rgba(255,255,255,0.18)" : "rgba(61,26,120,0.08)",
            ...
            <span style={{
                ...
                background: hov ? "#fff" : "rgba(61,26,120,0.07)",
                padding: "0.3rem 0.8rem", borderRadius: 50,
                letterSpacing: "0.06em",
                display: "inline-flex", alignItems: "center", gap: "0.4rem",
                alignSelf: "flex-start",
                border: `1.5px solid ${hov ? "rgba(255,255,255,0.9)" : "rgba(61,26,120,0.15)"}`,
                    ...
                    <div className="proc-right-row">
                        {STEPS.map((s, i) => (
                            <button key={s.num} data-hover onClick={() => setActiveStep(i)}
                                ...
                                style={{
                                    border: `1.5px solid ${activeStep === i ? "rgba(255,60,40,0.65)" : "rgba(255,255,255,0.1)"}`,
                                    background: activeStep === i ? "rgba(255,60,40,0.12)" : "rgba(255,255,255,0.04)",
                                    cursor: "none",
                                }}>
...
function HeroServicePanel() {
    const SVCS = [
        {
            label: "Web Development", color: "#FF8C00",
        ...
        {
            label: "UI / UX Design", color: "#FFBA08",
        ...
        {
            label: "WordPress & CMS", color: "#FFA000",
    ...
    return (
        <div className="hero-svc-panel">
            ...
            <div style={{ position: "absolute", inset: 0, pointerEvents: "none", backgroundImage: `radial-gradient(rgba(255,200,100,0.25) 1px, transparent 1px)`, backgroundSize: "26px 26px", opacity: 0.7 }} />
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${ac}, #FFBA08, transparent)`, opacity: 0.9 }} />
            ...
            <div className="hero-svc-inner">
                ...
                <div className="svc-progress-bar">
                    <div key={`${active}-${paused}`} className="svc-progress-fill" style={{ animation: paused ? "none" : "qg-prog 4s linear forwards", background: `linear-gradient(90deg, ${ac}, #FFBA08)`, boxShadow: `0 0 8px ${ac}` }} />
                </div>
                <div className="svc-btn-grid">
                    {SVCS.map((s, i) => (
                        <button key={s.label} data-hover onClick={() => { setActive(i); setPaused(true); }}
                            style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "clamp(0.55rem, 1.1vw, 0.65rem)", letterSpacing: "0.01em", padding: "0.55rem 0.4rem", borderRadius: 8, cursor: "none", textAlign: "center", lineHeight: 1.3, background: i === active ? `${s.color}22` : "rgba(255,255,255,0.05)", border: `1.5px solid ${i === active ? s.color : "rgba(255,220,100,0.15)"}`, color: i === active ? s.color : "rgba(255,210,120,0.55)", boxShadow: i === active ? `0 0 14px ${s.color}35` : "none", transition: "all 0.25s" }}>
                            {s.label}
                        </button>
                    ))}
                </div>
                <div key={active} className="svc-main-card">
                    <span className="glow-line top" style={{ background: `linear-gradient(90deg, transparent, ${ac}, #FFBA08, transparent)`, boxShadow: `0 0 10px ${ac}` }} />
                    <span className="glow-line right" style={{ background: `linear-gradient(180deg, transparent, ${ac}, #FFBA08, transparent)`, boxShadow: `0 0 10px ${ac}` }} />
                    <span className="glow-line bottom" style={{ background: `linear-gradient(270deg, transparent, ${ac}, #FFBA08, transparent)`, boxShadow: `0 0 10px ${ac}` }} />
                    <span className="glow-line left" style={{ background: `linear-gradient(0deg, transparent, ${ac}, #FFBA08, transparent)`, boxShadow: `0 0 10px ${ac}` }} />
                    <div>
                        <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(0.85rem,1.8vw,1rem)", color: "#FFEECC", marginBottom: "0.3rem" }}>{cur.label}</div>
                        <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: "clamp(0.72rem,1.2vw,0.82rem)", color: "rgba(255,230,160,0.75)", lineHeight: 1.55 }}>{cur.desc}</div>
                    </div>
...
function HomePage({ setPage, goToService }) {
    ...
    const INDUSTRIES = [
        { icon: "🛒", name: "Retail & E-Commerce", desc: "High-converting stores with seamless product browsing and blazing-fast checkout.", tag: "E-Commerce" },
    ...
    return (
        <div style={{ width: "100%", minWidth: 0, paddingTop: 70 }}>
                ...
                <div className="why-grid">
                    <Reveal style={{ height: "100%" }}>
                        <div style={{ background: C.purple, borderRadius: 24, padding: "3rem", position: "relative", overflow: "hidden", boxSizing: "border-box", display: "flex", flexDirection: "column", height: "100%" }}>
                            <div style={{ position: "absolute", bottom: "-1.5rem", right: "-0.5rem", fontFamily: "'Bebas Neue',sans-serif", fontSize: "clamp(5rem,12vw,9rem)", color: C.cream, opacity: 0.04, userSelect: "none", pointerEvents: "none", lineHeight: 1 }}>QG</div>
                            <div style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", background: "rgba(255,60,40,0.12)", border: "1px solid rgba(255,60,40,0.18)", borderRadius: 50, padding: "0.35rem 0.9rem", width: "fit-content", marginBottom: "2rem" }}>
                                <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.coral, animation: "breathe 2s ease infinite" }} />
                                <span style={{ fontFamily: "'Syne',sans-serif", fontSize: "0.65rem", fontWeight: 700, color: C.coral, letterSpacing: "0.12em", textTransform: "uppercase" }}>Why QwikGen</span>
                            </div>
            ...
            {/* ══ MARQUEE 2 ══ */}
            <Marquee items={MQ2} coral reverse />

            {/* ══ OUR APPROACH ══ */}
            <section className="qg-section" style={{ background: C.purple, position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse at 20% 50%,rgba(255,60,40,0.06),transparent 50%),radial-gradient(ellipse at 80% 50%,rgba(107,63,168,0.3),transparent 50%)" }} />
                <div style={{ position: "absolute", right: 0, top: "50%", transform: "translateY(-50%)", fontFamily: "'Bebas Neue',sans-serif", fontSize: "clamp(4rem,9vw,8rem)", color: "transparent", WebkitTextStroke: "1px rgba(255,255,255,0.06)", userSelect: "none", pointerEvents: "none", lineHeight: 0.85, textAlign: "right", overflow: "hidden", maxWidth: "35%" }}>QWIK<br />GEN</div>
                ...
                <div style={{ position: "relative", zIndex: 2, paddingBottom: "2.5rem", borderBottom: "1px solid rgba(255,255,255,0.08)", marginBottom: "2.5rem" }}>
                    <Reveal>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", background: "rgba(255,60,40,0.12)", border: "1px solid rgba(255,60,40,0.22)", borderRadius: 50, padding: "0.4rem 1.1rem", marginBottom: "1.2rem" }}>
                            <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.coral, animation: "breathe 2s ease infinite" }} />
