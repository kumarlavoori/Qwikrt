...
function Marquee({ items, coral = false, reverse = false }) {
    ...
    return (
        <div style={{ background: coral ? C.coral : C.creamDark, padding: "0.85rem 0", overflow: "hidden" }}>
            <div style={{ display: "flex", gap: "2.5rem", whiteSpace: "nowrap", width: "max-content", animation: `${reverse ? "marqueeRev" : "marquee"} 32s linear infinite` }}>
                {tripled.map((item, i) => (
                    <span key={i} style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "0.76rem", color: coral ? "rgba(255,255,255,0.85)" : C.inkSoft, letterSpacing: "0.1em", textTransform: "uppercase", flexShrink: 0, display: "inline-flex", alignItems: "center", gap: "0.6rem" }}>
                        {item}<span style={{ color: coral ? "rgba(255,255,255,0.45)" : C.coral, fontSize: "0.45rem" }}>●</span>
                    </span>
                ))}
            </div>
        </div>
...
function ScanLine() {
    return (
        <div style={{
            ...
            background: "linear-gradient(90deg, transparent, rgba(255,60,40,0.12), transparent)",
                ...
                <div className="svc-explorer-grid">
                        ...
                        {SERVICES.map((s, i) => (
                            <button
                                ...
                                style={{
                                    ...
                                    boxShadow: active === i ? "inset 0 0 0 1px rgba(255,60,40,0.07)" : "none",
                                    fontFamily: "inherit",
                                }}
                            >
                                <span style={{ position: "absolute", inset: 0, background: "linear-gradient(90deg, rgba(255,60,40,0.05), transparent)", opacity: active === i ? 1 : 0, transition: "opacity 0.3s", pointerEvents: "none" }} />
                                <span style={{ background: active === i ? C.coral : "transparent", borderRadius: 6, padding: active === i ? "2px 6px" : "2px 2px", transition: "all 0.28s cubic-bezier(0.16,1,0.3,1)", flexShrink: 0 }}>
                                    <span className="svc-list-num" style={{ color: active === i ? "#fff" : "rgba(107,96,128,0.45)" }}>{s.num}</span>
                                </span>
                    ...
                    <div className="svc-explorer-panel" key={animKey}
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.2rem", position: "relative", zIndex: 2 }}>
                            ...
                            <span style={{ fontFamily: "'Syne',sans-serif", fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", background: "rgba(255,60,40,0.09)", border: "1.5px solid rgba(255,60,40,0.22)", color: "#C03000", padding: "0.22rem 0.75rem", borderRadius: 50 }}>{cur.tag}</span>
                        </div>
            ...
            <div className="svc-dots-row">
                {SERVICES.map((_, i) => (
                    <button key={i} data-hover onClick={() => handleSelect(i)}
                        style={{ width: active === i ? 28 : 8, height: 8, borderRadius: 50, border: "none", cursor: "none", background: active === i ? C.coral : C.creamDark, boxShadow: active === i ? "0 0 8px rgba(255,60,40,0.5)" : "none", transition: "all 0.3s ease" }} />
                ))}
            </div>
        </section>
    );
    ...
    return (
        <div ref={ref} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            ...
            <div style={{
                ...
                boxShadow: isCurrent ? "0 10px 40px rgba(255,60,40,0.13), 0 2px 12px rgba(61,26,120,0.06)" : hov ? "0 8px 28px rgba(61,26,120,0.09)" : "0 2px 10px rgba(61,26,120,0.04)",
                ...
                <div>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.55rem", marginBottom: "0.65rem" }}>
                        <div style={{ width: isMobile ? 34 : 46, height: isMobile ? 34 : 46, borderRadius: 10, flexShrink: 0, background: isActive ? "rgba(255,60,40,0.07)" : C.creamDark, border: `1.5px solid ${isActive ? "rgba(255,60,40,0.15)" : C.creamDark}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: isMobile ? "1rem" : "1.35rem", animation: isCurrent ? "iconFloat 3s ease infinite" : "none", transition: "all 0.4s" }}>{step.icon}</div>
                        <span style={{ fontFamily: "'Syne',sans-serif", fontSize: isMobile ? "0.55rem" : "0.6rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", background: isActive ? "rgba(255,60,40,0.07)" : C.creamDark, color: isActive ? C.coral : C.inkSoft, padding: "0.18rem 0.6rem", borderRadius: 50, border: `1px solid ${isActive ? "rgba(255,60,40,0.15)" : C.creamDark}`, transition: "all 0.4s" }}>{step.phase}</span>
                    </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "0.32rem", marginTop: "0.4rem" }}>
                        {(STEP_BADGES[step.num] || ["⚡ Fast", "🔒 Transparent", "📈 Scalable"]).map((label, bi) => (
                            <div key={label} style={{
                                background: isActive
                                    ? "linear-gradient(135deg, rgba(255,60,40,0.06), rgba(61,26,120,0.06))"
                                    : C.creamDark,
                                border: `1px solid ${isActive ? "rgba(255,60,40,0.18)" : C.creamDark}`,
                                ...
                                boxShadow: isActive ? "0 2px 8px rgba(255,60,40,0.08)" : "none",
    ...
    if (isMobileWidth) {
        ...
        return (
            <div style={{ width: "100%", height: H, position: "relative", pointerEvents: "none" }}>
                ...
                <div style={{ position: "absolute", left: startX - trackW / 2, top: 0, width: trackW, height: `${fillPct}%`, background: "linear-gradient(180deg, #FF3C28, #6B3FA8)", borderRadius: 4, boxShadow: fillPct > 0 ? "0 0 8px rgba(255,60,40,0.5)" : "none", transition: "height 0.5s ease-out" }} />
            </div>
        );
    }
            ...
            <svg width={containerWidth} height={H} style={{ display: "block", overflow: "visible" }}>
                <defs>
                    <linearGradient id={gradId} x1="0%" y1="0%" x2="100%" y2="100%">
                        <stop offset="0%" stopColor="#FF3C28" />
                ...
                <path d={pathD} fill="none" stroke={`url(#${gradId})`} strokeWidth="6" strokeLinecap="round"
                    ...
                    style={{ filter: "drop-shadow(0 0 5px rgba(255,60,40,0.5))", transition: "stroke-dashoffset 0.5s ease-out" }}
                />
            </svg>
        </div>
    );
    ...
    return (
        <div style={{ width: "100%", minWidth: 0, paddingTop: 70 }}>
            ...
            <section style={{
                ...
                backgroundImage: "radial-gradient(circle at 80% 30%, rgba(255,60,40,0.10), transparent 55%), radial-gradient(circle at 10% 90%, rgba(107,63,168,0.18), transparent 50%)",
            }}>
                        ...
                        <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap", marginTop: "2rem" }}>
                            {["React", "Next.js", "Flutter", "WordPress", "Shopify", "Figma", "Node.js", "Django"].map(t => (
                                <span key={t} style={{
                                    ...
                                    padding: "0.3rem 0.9rem", borderRadius: 50, color: "#FFBA08", letterSpacing: "0.06em",
                                }}>{t}</span>
                            ))}
                        </div>
                    </Reveal>
                    ...
                    <div style={{
                        ...
                        <button
                            ...
                            onMouseEnter={e => {
                                ...
                                e.currentTarget.style.boxShadow = "0 16px 40px rgba(255,60,40,0.5)";
                            }}
                            onMouseLeave={e => {
                                e.currentTarget.style.background = C.coral;
                                e.currentTarget.style.transform = "";
                                e.currentTarget.style.boxShadow = "0 6px 24px rgba(255,60,40,0.35)";
                            }}
                            style={{
                                fontFamily: "'Syne',sans-serif",
                                fontWeight: 700,
